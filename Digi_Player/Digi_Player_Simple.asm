...
NMI_HANDLER        
         ...
         LDA $DD0D              ; 4- (20)clear NMI

         ;every other NMI do *1 or *2
         LDA flag               ; 4- if flag==0 we just played upper nibble
         BNE lower              ; 2- (6) so skip ahead to load new byte

upper    LDA sample             ; 4- *1 shift upper nibble down
         LSR a                  ; 2-
         LSR a                  ; 2-
         LSR a                  ; 2-
         LSR a                  ; 2-
...
lower    LDY #0                 ; 2- *2 get a new packed sample byte
         LDA (ptr),y            ; 5-      
         STA sample             ; 4- save to temp location
         INC ptr                ; 5- inc point to next sample byte
         BNE checkend           ; 2- did we roll low byte over to zero?
         INC ptr+1              ; 5- (23) if so inc the high byte of pointer too
   
checkend LDA ptr                ; 3- if not @ end of sample exit/return from NIM
...
exit     
         LDA flag               ; 4- toggle hi/low nibble flag and exit NMI
         EOR #1                 ; 3-
         STA flag               ; 4- (11)
         ...
         TAX                    ; 2- 
         PLA                    ; 3-
         RTI                    ; 6- (19) return from this pass of NMI
                                ; 88 upper 704K, 100 lower 800K
